  ...
  return (
    <motion.header
      ...
      className={cn(
        ...
        /* Always frosted — ensures readability at every scroll position */
        scrolled
          ? "bg-white/95 backdrop-blur-xl shadow-soft border-b border-slate-200/60"
          : "bg-white/80 backdrop-blur-lg shadow-[0_1px_0_rgba(0,0,0,0.04)]"
      )}
    >
        ...
        <nav className="hidden lg:flex items-center gap-6">
          {NAV.map((l) => (
            <a
              key={l.href}
              href={l.href}
              className="text-[0.84rem] font-medium text-slate-600 hover:text-brand-600 transition-colors"
            >
              {l.label}
            </a>
          ))}
          ...
          {/* Social icons */}
          <div className="flex items-center gap-2 ml-1 pl-4 border-l border-slate-200">
            {SOCIALS.map((s) => {
              const Icon = s.icon;
              return (
                <a
      ...
      {open && (
        <motion.nav
          ...
          className="lg:hidden bg-white/95 backdrop-blur-xl border-b border-slate-100"
        >
          <div className="section-container py-4 flex flex-col gap-3">
            {NAV.map((l) => (
              <a
                ...
                className="text-sm font-medium text-slate-600 hover:text-brand-600 py-1"
              >
                {l.label}
              </a>
            ))}
            {/* Social row */}
            <div className="flex items-center gap-3 py-2 border-t border-slate-100 mt-1 pt-3">
              {SOCIALS.map((s) => {
                const Icon = s.icon;
                return (
                  <a
